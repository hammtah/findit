...
export const authApi = {
  ...
  forgotPassword: async (email: string): Promise<void> => { await apiClient.post('/auth/forgot-password', { email }); },
  resetPassword: async (data: ResetPasswordPayload): Promise<void> => { await apiClient.post('/auth/reset-password', data); },
  verifyEmail: async (token: string): Promise<LoginResponse> => (await apiClient.get<LoginResponse>(`/auth/verify-email?token=${encodeURIComponent(token)}`)).data,
};
